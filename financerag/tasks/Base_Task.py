...
class BaseTask:
    ...
    def __init__(self, metadata: TaskMetadata, data_path: str):
        """
    ...
    def load_data(self):
        ...
        #     ).load()
            ...
            if True: # 여기에 테이블 Task는 따로?
                query_path = os.path.join(self.data_path, 'queries.jsonl')
            ...
            self.queries = {query["_id"]: query["text"] for query in queries}
            self.corpus = {
                doc["_id"]: {"title": doc["title"], "text": doc["text"]}
                for doc in corpus
            }
    ...
    def evaluate(
        ...
        for k in ndcg.keys():
            logger.info("{}: {:.4f}".format(k, ndcg[k]))
